...
public class CollisionHandler : MonoBehaviourPun
{
    ...
    // 캐릭터 자신의 몸에 자신을 제외한 다른 InteractableObject가 충돌하면 실행되는 함수, 충돌량을 계산하여 피해를 주고 넉백시킨다.
    // 마스터 클라이언트의 OnCollisionEnter 내부에서 호출됨
    ...
    // 피격 부위별 데미지배율 변환 (ex. 내 머리에 충돌이 가해지면 받는 데미지가 증가)
    ...
    {
        // 충돌한 오브젝트가 TriggerObject고, 현재 내 플레이어가 무적상태가 아니고,Burn 상태가 아니어야 True로 통과
        // Burn만 중복 체크하는게 맞나? 
    ...
    private void TriggerCheck(Collider other)
    {
        ...
        // 이 조건이 맞나?
        // 충돌한 아이템의 오너가 자신이면 리턴(이건 정상)
        // 충돌한 오브젝트에 Item 컴포넌트가 있으면 리턴?
    ...
    // 넉백량 계산 및 처리 함수(해당 오브젝트의 photonView ID와 충돌 방향, 데미지 관련 변수들을 받아서 처리)
    [PunRPC]
