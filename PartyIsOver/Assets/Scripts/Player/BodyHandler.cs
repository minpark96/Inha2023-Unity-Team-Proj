...
public class BodyHandler : MonoBehaviourPun
{
    ...
    //기절후 회복기능, 관절 스프링을 천천히 올려서 기절에서 회복되는 연출
    ...
    // 기절 했을때의 몸이나 손으로 뭔가를 잡았을때 팔을 그대로 고정시키는 함수
    ...
    // 공격할때 특정 부위에 공격 속성을 부여 (ex. 펀치 동작중엔 양 팔에 isAttack을 true로)
    // 이 때 마스터 클라이언트에서만 변경하고 로컬 클라이언트는 변경하지 않음
    ...
    // 신체 부위에 공격 속성이 부여되면 공격력 계산을 위해 enum 값을 Default에서 해당하는 부위로 변경
    // (ex. 펀치 동작중엔 양 팔의 속성을 Punch로 변경, 공격이 끝나면 다시 Default로 변경)
    [PunRPC]
