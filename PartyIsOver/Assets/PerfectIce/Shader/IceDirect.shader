    ...
    SubShader {
        ...
        Pass {
            ...
            UNITY_INSTANCING_BUFFER_START( Props )
                ...
                UNITY_DEFINE_INSTANCED_PROP( float4, _ReflectColor)
                UNITY_DEFINE_INSTANCED_PROP( float, _RefStrength)
                UNITY_DEFINE_INSTANCED_PROP( float, _LightStrength)
                UNITY_DEFINE_INSTANCED_PROP( float, _FrenelPower)
                UNITY_DEFINE_INSTANCED_PROP( float, _TexAlphaAdd)
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float2 sceneUVs = (i.projPos.xy / i.projPos.w) + (_Refraction_var.rgb.rg*(_RefractionStrength_var*0.5));
                float4 sceneColor = tex2D(_GrabTexture, sceneUVs);
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(i.uv0, _MainTex));
                float _Cutoff_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Cutoff );
                clip((_MainTex_var.a+_Cutoff_var) - 0.5);
...
////// Specular:
                ...
                float3 specularColor = float3(_Shininess_var,_Shininess_var,_Shininess_var);
...
////// Emissive:
                ...
                float _LightStrength_var = UNITY_ACCESS_INSTANCED_PROP( Props, _LightStrength );
                float _RefStrength_var = UNITY_ACCESS_INSTANCED_PROP( Props, _RefStrength );
                float4 _ReflectColor_var = UNITY_ACCESS_INSTANCED_PROP( Props, _ReflectColor );
                float3 emissive = ((_MainTex_var.rgb+(lerp((_MainTex_var.rgb*_Color_var.rgb),(_Cube_var.rgb*_Cube_var.a),pow(_FrenelPower_var,lerp(-1,0,pow(1.0-max(0,dot(normalDirection, viewDirection)),0.5*dot(1.0,_Refraction_var.rgb)+0.5))))*_LightStrength_var*_RefStrength_var))*_ReflectColor_var.rgb);
/// Final Color:
                float3 finalColor = diffuse + specular + emissive;
                float _TexAlphaAdd_var = UNITY_ACCESS_INSTANCED_PROP( Props, _TexAlphaAdd );
                return fixed4(lerp(sceneColor.rgb, finalColor,(_MainTex_var.a+_TexAlphaAdd_var)),1);
            }
            ENDCG
        }
        Pass {
...
/// Final Color:
                ...
                float _TexAlphaAdd_var = UNITY_ACCESS_INSTANCED_PROP( Props, _TexAlphaAdd );
                return fixed4(finalColor * (_MainTex_var.a+_TexAlphaAdd_var),0);
            }
            ENDCG
        }
        Pass {
